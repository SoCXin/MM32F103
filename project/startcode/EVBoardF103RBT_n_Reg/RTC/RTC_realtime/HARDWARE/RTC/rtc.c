...
u8 RTC_Init(void)
{
    ...
    RCC->APB1ENR |= RCC_APB1RSTR_BKPRST;   //使能备份时钟
    PWR->CR |= PWR_CR_DBP;         //取消备份区写保护
    if(BKP->DR1 != 0X5050) //第一次配置
    {
        RCC->BDCR |= RCC_BDCR_LSEON;       //开启外部低速振荡器
        while((!(RCC->BDCR & RCC_BDCR_LSERDY)) && temp < 250) //等待外部时钟就绪
        {
            temp++;
        };
        if(temp >= 500)return 1; //初始化时钟失败,晶振有问题
        RCC->BDCR |= RCC_BDCR_RTCSEL_LSE; //LSE作为RTC时钟
        delay_us(1);//小延时，等待同步
...
{
    if(RTC->CRL & RTC_CRL_SECF) //秒钟中断
    {
        RTC_Get();//更新时间
    }
    if(RTC->CRL & RTC_CRL_ALRF) //闹钟中断
    {
        RTC->CRL &= ~(RTC_CRL_ALRF);		//清闹钟中断
    }
